...
\begin{equation}
  ...
  \label{eq:cid-state}
\end{equation}
where $F_s$ is the fact channel, $T_s$ the thought channel, $Y_s$ the display channel, $\mathcal{B}_s$ the active perceptual bindings, and $\mathcal{J}_s$ the in-flight external jobs. The original prompt $P$ is fixed token-level conditioning across the trajectory rather than a fourth mutable channel; explicitly protected prompt content may additionally be represented in $F_s$. The model updates $T_s$ and $Y_s$, while the environment and runtime update $F_s$, $\mathcal{B}_s$, and $\mathcal{J}_s$ under explicit permissions.
...
\begin{figure}[t]
  \centering
  \resizebox{0.98\columnwidth}{!}{\input{figures/tct-anatomy}}
  \caption{Anatomy of a Typed Cognitive Tensor cell. Continuous semantics remain locally revisable, while typed metadata exposes exact anchors, dependencies, uncertainty, editability, and lifecycle to the runtime.}
...
\begin{figure*}[t]
  \centering
  \resizebox{0.96\textwidth}{!}{\input{figures/binding-lifecycle}}
  \caption{Lifecycle of a persistent perceptual binding. An unchanged source can be cognitively refreshed by re-projecting its cached value, while a source-version change triggers a new external refresh before projection. The binding persists until the originating need becomes inactive.}
...
\begin{figure*}[t]
  \centering
  \resizebox{0.96\textwidth}{!}{\input{figures/runtime-loop}}
  \caption{Reference CID runtime loop for read-only perception. Model denoising, binding management, cached re-projection, and external I/O advance as coupled but separately scheduled processes.}
  \label{fig:cid-runtime}
\end{figure*}

Initialization sets facts $F$, masked thought $T$, masked display $Y$, bindings $\mathcal{B}=\varnothing$, and jobs $\mathcal{J}=\varnothing$. Each model update exposes information needs; the runtime matches or updates bindings, launches a read only when policy requires a fresh value and no equivalent job is active, consumes arriving events, and re-projects cached values when another external access is unnecessary. The model then adjusts local noise using support or conflict impact and continues until convergence or budget exhaustion. The runtime deduplicates equivalent external work but does not suppress repeated perception: two identical needs may share one cached value while each diffusion step obtains a fresh projection. A refined need can update an existing binding's selector, while an independent-verification need can establish a second source binding.
