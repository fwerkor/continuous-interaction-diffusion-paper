...
\begin{equation}
  f_j = (v_j, \kappa_j, t_j, \nu_j, p_j),
\end{equation}
where $v_j$ is the value, $\kappa_j$ its source type, $t_j$ a timestamp, $\nu_j$ a version or freshness marker, and $p_j$ provenance. Fact-channel policy is an explicit runtime decision governing whether a tool result is promoted.
...
If the source is time-varying or streamable, $\rho_j$ may request polling, version checks, event subscriptions, or incremental chunks. An arriving value updates a fact item if policy marks it as protected, and always permits a new cognitive projection. Examples include current time, evolving realized display length, a modified file, sensor input, or streaming retrieval results.
...
\end{equation}
where $\Delta^-_{j,i}$ measures conflict and $\Delta^+_{j,i}$ support. Conflicting cognitive cells become more diffused and revisable; supported cells stabilize. Evidence-linked display positions are reopened analogously through $\tau^Y_s$, implemented by remasking or bounded visible-token revision.
...
\Cref{fig:cid-runtime} describes the runtime loop. Model denoising does not block merely because a read-only operation is outstanding: it continues while useful refinement remains under the currently available information. A learned convergence signal instead represents \emph{current-information equilibrium}: no further model update is expected to be useful until either external information changes or the trajectory is finalized. If this state is reached while a required binding is still waiting, the runtime enters quiescence, issues no further model update, and resumes the same trajectory when external progress arrives. If no required binding is outstanding, equilibrium produces a terminal candidate that must pass the freshness barrier below.
...
Initialization sets facts $F$, an initial thought state $T$, masked display $Y$, bindings $\mathcal{B}=\varnothing$, and jobs $\mathcal{J}=\varnothing$. Each model update exposes information needs; the runtime matches or updates bindings, launches a read only when policy requires a fresh value and no equivalent job is active, consumes arriving events, and re-projects cached values when another external access is unnecessary. The model then adjusts local noise using support or conflict impact and continues until it reaches current-information equilibrium, a terminal state, or a budget boundary. Quiescent waiting does not consume model-update budget; an external observation reopens refinement. A terminal candidate additionally passes a \emph{final freshness barrier}: unresolved required bindings must complete, and bindings whose freshness policy requires a current value are refreshed or version-checked before the display is sealed. If this check reveals new evidence, the affected cognition and display remain revisable and the trajectory resumes.

External work is keyed by the source and canonicalized launch arguments, and a completion is applied only to active bindings whose current work key still matches it; selector refinement therefore makes obsolete in-flight work cancellable or discardable. The runtime deduplicates equivalent external work but does not suppress repeated perception: two identical needs may share one cached value while each diffusion step obtains a fresh projection. A refined need can update an existing binding's selector, while an independent-verification need can establish a second source binding. The step cap bounds a refinement epoch rather than elapsed tool time, while a separate wall-clock budget bounds the complete trajectory.
...
Training randomizes event arrival, source freshness, and cache state. Before a needed event arrives, the model should maintain uncertainty and expose a binding need rather than fabricate a value. Waiting snapshots that have exhausted useful source-independent refinement supervise the convergence head as current-information equilibria; final snapshots are positive for the same head, while runtime gates distinguish quiescence from termination. After arrival, the model should assimilate the percept, revise affected cells, preserve unrelated cognition, and update the display.
