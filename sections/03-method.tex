...
$F_s$ contains information whose value is controlled outside the generative state. The model can attend to it but cannot overwrite it. The channel may still change over time:
\begin{equation}
  F_{s+1} = \operatorname{ExternalUpdate}(F_s, e_{s+1}).
  \label{eq:fact-update}
\end{equation}
A clock, system load, changing document, or user-updated constraint may therefore change between diffusion steps. The channel may contain user-pinned statements, exact values that must be quoted, authoritative tool results, and provenance metadata. Tool results may remain in the runtime cache and enter cognition through a temporary projection when promotion to $F_s$ is unnecessary. The registered source descriptor declares promotion permission: a source may mark its returned observations as protected, and only the runtime performs the corresponding fact-store update.
...
\begin{figure}[htbp]
  \centering
  \input{figures/tct-anatomy}
  \caption{A single TCT cell groups semantic content and typed attributes within one boundary (schematic).}
...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
For read-only tools, a binding may begin before every argument has reached textual certainty, provided the source accepts approximate or progressively refined selectors. Side-effecting actions may expose and refine intent equally early, but external execution should remain behind a runtime-controlled commit boundary.

\begin{figure*}[t]
  \centering
  \input{figures/binding-lifecycle}
...
CID extends a transient request--response pair into a persistent perceptual binding
\begin{equation}
...
A binding remains active while the corresponding information need remains active. Repeated intent can keep the percept cognitively salient across denoising steps. The runtime distinguishes two operations:
\begin{align}
...
\begin{figure*}[t]
  \centering
  \input{figures/runtime-loop}
  \caption{CID runtime loop in the read-only evaluation setting.}
...
\end{equation}
where $\Delta^-_{j,i}$ denotes evidence favoring reopening and $\Delta^+_{j,i}$ evidence favoring stabilization. The current implementation directly predicts a bounded local-noise delta together with a \textsc{keep}, \textsc{reopen}, or \textsc{stabilize} action through the neural core, with the runtime lifecycle gate applying the legal transition and no separate support/conflict heads. Evidence-linked display positions are revised analogously through the learned $\chi_k$ routing, remasking, and bounded visible-token revision.

Local reopening is important for efficiency as well as correctness. New evidence should revise invalidated state while preserving unrelated work that was already correct. Adjusting editability per cell therefore aims to preserve useful computation while concentrating additional denoising on regions that actually need correction. \Cref{fig:selective-revision} illustrates this selective update using the running example.

\begin{figure*}[t]
  \centering
  \input{figures/selective-revision}
  \caption{Illustrative local revision: evidence reopens a conflicting hypothesis, grounds a claim, and updates its display span while preserving unrelated state. Cell labels describe latent content, not decoded reasoning.}
  \label{fig:selective-revision}
\end{figure*}

\subsection{Asynchronous Runtime}
\label{sec:runtime}

\Cref{fig:cid-runtime} describes the runtime loop in the evaluated read-only setting. Model denoising may continue while external work is outstanding until a learned \emph{current-information equilibrium} signal fires. The runtime then pauses if required work remains and resumes on external progress; otherwise it forms a terminal candidate.

In the evaluated setting, the runtime uses retrieval, file-I/O, and other read-only waiting periods for source-independent refinement. CID continues refining cognition and display regions while the job is in flight, then revises dependent regions after arrival. End-to-end latency improves when this waiting-time computation remains useful, so the evaluation measures end-to-end latency at matched answer quality and treats token-generation speed as an insufficient metric on its own.
...
CID requires a model trained for channel semantics and asynchronous events. A training trajectory contains a task $x$, protected facts $F$, tool/source descriptors $\mathcal{D}$, external event sequence $E=(e_1,\ldots,e_K)$ with arrival times, target thought structures $T^*$ or weak structural supervision, and final display $Y^*$. Each supervised cognitive cell carries a concise semantic summary $q_i$ together with typed targets such as role, uncertainty, local diffusion state, lifecycle, and interaction fields. The summary is training-time supervision, not text that the runtime asks the model to generate.

For the continuous semantic field, the text encoder maps $q_i$ to a detached target vector $e_i^*=\operatorname{sg}(E_{\mathrm{sem}}(q_i))\in\mathbb{R}^d$. Given a corrupted or self-rolled thought state, the neural core predicts the corresponding TCT vector $\hat h_i$. Over the supervised cell set $\mathcal{M}$, the semantic thought loss is
\begin{equation}
  \mathcal{L}_{T}=\frac{1}{|\mathcal{M}|}\sum_{i\in\mathcal{M}}\lVert \hat h_i-e_i^*\rVert_2^2.
  \label{eq:thought-semantic-loss}
\end{equation}
Thus natural-language summaries provide semantic coordinates for the latent cognitive field during training, while inference keeps the field continuous and need not decode an intermediate chain of thought. Separate heads supervise allocation, roles, uncertainty and local noise, lifecycle/revision, information needs, source and argument binding, affected-region routing, grounding, and convergence.

Training varies event-arrival schedules and source-freshness demands across trajectories. Before a needed event arrives, the model should maintain uncertainty, expose a binding need, and avoid fabricating the missing value. Closed-loop targets are conditioned on the observations actually obtained by the rollout: thought and display supervision remain at an unresolved pre-observation state until each required event arrives. Training and runtime also share the same canonical unresolved display initialization, with an explicit EOS boundary followed by latent canvas capacity. Cache reuse remains runtime-owned and is exercised separately through runtime controls. Waiting and final snapshots supervise the equilibrium signal; runtime state distinguishes quiescence from termination. After arrival, the model should assimilate the percept, revise affected cells, preserve unrelated cognition, and update the display.
