...
\begin{table}[t]
  ...
  \begin{tabular}{p{0.19\linewidth}p{0.22\linewidth}p{0.28\linewidth}p{0.22\linewidth}}
    ...
    \midrule
    Provisional-state decoding & stale vs. provisional thought & display exact; Need F1; recovery failure & 13.6$\rightarrow$22.8; 1.9$\rightarrow$9.1; 8.14$\rightarrow$5.81 \\
    Asynchronous execution & async vs. forced blocking & latency; wait overlap; task quality & \textit{pending runtime replay} \\
    Continuous revision & normal vs. freeze after materialization & final accuracy; stale rate; repair rate & \textit{pending runtime replay} \\
    \bottomrule
  \end{tabular}
\end{table}
