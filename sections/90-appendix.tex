\section{Terminology}
\label{app:terminology}

The following definitions specify how potentially overloaded terms are used throughout this paper.

\begin{description}[leftmargin=0pt,labelindent=0pt,itemsep=2pt,topsep=3pt]
  \item[Autoregressive language model (AR LM).]
  A language model that generates an output by predicting each next token from the input and the already generated prefix. Standard decoding advances left to right and treats earlier emitted tokens as fixed context.

  \item[Diffusion language model (dLLM).]
  A language model that generates through iterative denoising or refinement of a partially unknown or corrupted sequence. Depending on the architecture, several positions may be updated in one step and uncertain earlier positions may remain revisable.

  \item[Denoising step / diffusion update.]
  One iterative update of the diffusion state. A diffusion update can modify multiple positions or cognitive cells and is therefore distinct from generating one token in an AR model.

  \item[Reopening / remasking.]
  Increasing the editability of a previously predicted region so that later computation or newly arrived evidence can revise it. Remasking is one concrete token-level mechanism for reopening.

  \item[Tool call.]
  A discrete executable request to an external source, normally containing a tool or source identity and sufficiently bound arguments such as a query or structured JSON fields.

  \item[Observation.]
  Information returned by an external source or runtime event and made available to the model.

  \item[Information need.]
  A model state indicating that external information would be useful. In CID, such a need may become available to the runtime before it has converged into a complete serialized tool call.

  \item[Perceptual binding.]
  A persistent runtime relation connecting an information need to an external source, its current arguments, refresh policy, cached state, and affected cognitive regions.

  \item[External refresh.]
  Re-executing or re-reading an external source to obtain a potentially newer value. External refresh incurs external I/O or source computation.

  \item[Cognitive refresh / re-projection.]
  Recomputing how an already available external value should influence the current cognitive state, without repeating external I/O.

  \item[Perceptual projection.]
  A context-dependent model representation of an external value, constructed with respect to the current thought and display states and optionally linked to its provenance.

  \item[Fact channel.]
  Externally controlled information that the model may read but may not overwrite. Its contents can still change when the user, environment, runtime, or source updates them.

  \item[Thought channel.]
  The revisable internal cognitive state containing hypotheses, plans, information needs, percept interpretations, uncertainty, constraints, and intermediate conclusions.

  \item[Display channel.]
  The revisable token canvas that progressively converges to the user-visible response.

  \item[Typed Cognitive Tensor (TCT).]
  CID's structured continuous representation of the thought channel. Cognitive cells combine semantic vectors with typed roles, symbolic anchors, source links, uncertainty, local diffusion levels, and lifecycle state.

  \item[Local diffusion level.]
  A per-region value controlling how revisable that region currently is. Higher diffusion permits stronger revision; lower diffusion represents a more stable region.

  \item[Persistent perception.]
  The maintenance of an information source's cognitive influence while the corresponding need remains active. Persistence may reuse a static cached value or refresh a changing source according to policy.

  \item[Static and dynamic sources.]
  A static source is treated as unchanged while a binding is active, allowing repeated cognitive refresh from a cached value. A dynamic source can change over time and may require polling, version checks, subscriptions, or streaming updates.

  \item[Binding lifecycle.]
  The runtime state of a perceptual binding as it moves through stages such as \textsc{candidate}, \textsc{active}, \textsc{waiting}, \textsc{available}, \textsc{refreshing}, and \textsc{retired}.
\end{description}

\section{Reference Runtime Interface}
\label{app:runtime-interface}

A read-only source registered with a CID runtime may expose the following conceptual schema:

\begin{verbatim}
source:
  name: document_search
  description: >-
    retrieve passages from a corpus
  input_schema:
    query: text
    scope: optional identifier
    top_k: optional integer
  properties:
    read_only: true
    cacheable: true
    streamable: true
    cancellable: true
    versioned: true
  default_refresh:
    mode: on_version_change
    max_age_ms: 1000
\end{verbatim}

Source descriptors support heterogeneous capabilities. A calculator may be cacheable without streaming support; a clock is dynamic and inexpensive; a file may expose a version identifier; a search endpoint may return incremental results without a stable version.

\section{Binding State Machine}
\label{app:binding-state}

A perceptual binding can use the following lifecycle:
\begin{align}
  \textsc{candidate} &\rightarrow \textsc{active}
  \rightarrow \textsc{waiting} \notag\\
  &\rightarrow \textsc{available}
  \rightarrow \textsc{refreshing} \notag\\
  &\rightarrow \textsc{retired}.
\end{align}
A binding can move from \textsc{available} directly back to \textsc{active} when the value is unchanged and only cognitive re-projection is required. It moves to \textsc{refreshing} when the need remains active and the source freshness policy requires external work. Quiescence is defined at the trajectory level: active bindings remain intact while model updates pause, and the next observation resumes refinement without reconstructing the interaction context.

A candidate binding should record:
\begin{itemize}[leftmargin=*]
  \item source probability and need confidence;
  \item partially bound selector or arguments;
  \item a stable owning cognitive cell and learned links to additional affected TCT/display regions;
  \item current cache key and source version;
  \item external and cognitive refresh policies;
  \item runtime-owned permission for whether returned values are promoted to the protected fact channel.
\end{itemize}

\section{Conceptual Execution Traces}
\label{app:traces}

\subsection{Static Copying}

A user requests a response that must reproduce a value from a document exactly. The model forms a persistent need for the value and binds to a file reader. After the file is read once, the runtime caches the exact result. During subsequent denoising steps, the unchanged result is re-projected into the thought tensor, preserving salience while the display is reorganized. No repeated disk I/O is necessary, but the percept remains active until the exact value has been anchored in the display.

\subsection{Dynamic Display Length}

The current realized display length---for example, the inferred EOS position or populated span within the fixed canvas---is an externally computed value that the model may read but cannot rewrite. A binding to the length monitor remains active during generation. At each update, the runtime supplies a refreshed value, which changes the thought tensor's plan and density decisions. The display can compress earlier text because the value conditions both current cognition and already generated output regions.

\subsection{Streaming Retrieval}

A retrieval source first returns titles, then snippets, then complete passages. The binding remains active throughout the stream. Early chunks narrow hypotheses and refine the query; later chunks reopen unsupported conclusions and supply exact evidence. Unrelated display regions remain stable under local diffusion clocks.

\section{Training Data Construction}
\label{app:data}

CID-Dataset is synthesized from ordinary question--answer pairs, documents, and controlled interaction tasks:
\begin{enumerate}[leftmargin=*]
  \item identify answer spans that depend on external evidence and hide those spans from the model-visible prompt;
  \item create source descriptors, binding targets, delayed or incremental arrival schedules, and source-level protected-result policy;
  \item annotate each need with one stable owner and any additional thought/display regions that should receive its percept;
  \item construct pre-arrival states with unresolved cells and post-arrival revisions grounded in the source;
  \item add static-copy cases that preserve one value across many steps and dynamic cases in which the source changes before display convergence.
\end{enumerate}

Teacher-generated supervision is stored as concise semantic state summaries; private chain-of-thought transcripts are excluded. During tensorization, each summary is encoded into a detached semantic target vector for its cognitive cell, while roles, uncertainty, lifecycle/revision, interaction needs, routing, and related typed fields receive their own targets. The summary text itself is not a runtime output. Training randomizes semantic presentation and event schedules without introducing arbitrary physical-slot labels into the semantic target. CID v1 tensorizes every transition into the same 128-slot physical field used at runtime; training assigns stable logical cells to deterministic slots within a trajectory, while runtime-owned reclamation and compaction may later move physical positions without changing cell identity.

\section{Experimental Reproducibility}
\label{app:reproducibility}

\subsection{CID-Dataset Training Corpus}

Formal training uses the current public \texttt{fwerkor/CID-Dataset} release~\cite{cao2026ciddataset}. The canonical training materialization contains 192,729 semantic tasks expanded into 422,230 trajectories, with 2,724,556 adjacent-state transitions and 422,230 empty-state bootstrap transitions, for 3,146,786 trainable transitions. Maximum trajectory length is 44 snapshots and the maximum required TCT capacity is 128. Semantic-task balancing is applied before component weights, and the global transition-weight mean is normalized to one. Of the 192,729 semantic tasks, 135,086 (70.09\%) require tools, 43,981 (22.82\%) require no tool, and 13,662 (7.09\%) expose tools that are unnecessary. The latter two groups provide negative supervision against indiscriminate tool use.

\begin{table*}[t]
  \centering
  \scriptsize
  \begin{tabular}{lrrrr}
    \toprule
    Component & Tasks & Traj. & Train trans. & Weight \\
    \midrule
    public-base & 8,512 & 34,048 & 105,304 & 2.0 \\
    public-interaction & 7,590 & 30,360 & 430,834 & 2.5 \\
    mechanism & 10,000 & 40,000 & 230,287 & 1.0 \\
    computational & 12,000 & 24,000 & 137,739 & 1.0 \\
    symbolic & 15,600 & 31,200 & 155,504 & 1.0 \\
    correction & 10,000 & 20,000 & 140,220 & 1.0 \\
    complex-logic & 12,000 & 24,000 & 168,000 & 1.0 \\
    self-identity & 720 & 1,440 & 4,280 & 1.0 \\
    multilingual & 1,200 & 2,400 & 24,125 & 1.0 \\
    composed & 12,000 & 24,000 & 261,058 & 1.0 \\
    tool-restraint & 6,500 & 6,500 & 13,000 & 1.5 \\
    compositional-longtail & 20,000 & 40,000 & 260,000 & 1.0 \\
    long-horizon & 12,000 & 24,000 & 471,626 & 1.0 \\
    deep-tool-restraint & 4,000 & 4,000 & 30,000 & 1.5 \\
    natural-interaction & 8,675 & 17,350 & 182,059 & 2.0 \\
    natural-public/nq-open & 30,000 & 60,000 & 330,121 & 1.0 \\
    natural-public/oasst1 & 4,500 & 4,500 & 13,500 & 1.5 \\
    natural-public/multidoc2dial & 15,000 & 30,000 & 165,171 & 1.5 \\
    natural-public/qasper & 2,000 & 4,000 & 21,942 & 2.0 \\
    curated & 432 & 432 & 2,016 & 4.0 \\
    \midrule
    Total & 192,729 & 422,230 & 3,146,786 & -- \\
    \bottomrule
  \end{tabular}
  \caption{CID-Dataset components used for CID training. Task counts are semantic tasks before schedule expansion; trajectory counts are materialized trajectories; training-transition counts include each component's bootstrap transitions. Weight is the semantic component multiplier applied after per-task balancing.}
  \label{tab:dataset-components}
\end{table*}

The natural-public component contributes 30,000 NQ-Open train tasks~\citep{kwiatkowski2019natural,lee2019latent} (CC-BY-SA-3.0), 4,500 OASST1 train
tasks~\citep{kopf2023openassistant} (Apache-2.0), 15,000 MultiDoc2Dial train tasks~\citep{feng2021multidoc2dial} (CC-BY-3.0), and 2,000 QASPER train tasks~\citep{dasigi2021qasper}
(CC-BY-4.0). Public-source rows retain upstream repository, revision, split, license, and row-key
provenance. Teacher-generated components contain concise semantic state summaries; private
chain-of-thought transcripts are excluded. Physical slot placement and event timing are schedule variables; they are not produced by the teacher.

\subsection{Final Evaluation Protocol}

Task families cover static reference/copying, dynamic state tracking, delayed retrieval, streaming
evidence, competing sources, natural open-domain questions, and multi-document reasoning. Baselines
include blocking ReAct; an event-driven asynchronous AR agent \citep{ginart2024asynchronous,hooper2026realtime};
a dLLM with conventional explicit-call regions; and P-ReAct/DLLM-Searcher \citep{zhao2026dllmsearcher}
when the corresponding source interface can be matched.

We report exact or task-specific correctness, evidence support, copying fidelity, stale-value rate,
wall-clock latency, tool-wait overlap, binding precision/recall, cache hits, external refreshes,
duplicate-I/O suppression, repeated cognitive projection, and revision locality. Intent lead time is
$\Delta_{\mathrm{lead}}=t_{\mathrm{explicit\ call}}-t_{\mathrm{binding}}$, and assimilation lag is
$\Delta_{\mathrm{assim}}=t_{\mathrm{correct\ state}}-t_{\mathrm{arrival}}$. Quality comparisons use
a logical event clock so accelerator speed cannot change pre-arrival refinement; matched-hardware
wall-clock measurements are reported separately.

Controlled runtime analyses reuse the same final checkpoint and avoid training separate weakened
models. The headline runtime ablations compare asynchronous against forced-blocking execution and
continuous revision against freeze-after-materialization; additional analyses cover persistent
re-projection against one-time percept injection, need-conditioned refresh against no refresh, cache
reuse against forced re-execution, and multiple source-latency schedules. These interventions isolate
runtime behavior at negligible training cost. We additionally retain one controlled development
retraining pair in which the base model, frozen CID-Dataset materialization, neural contract, batch
geometry, and optimization budget are matched and only provisional-state-aware interaction decoding
changes. This pair is reported as a mechanism ablation in \cref{sec:ablations}, not as a comparison to
the later final corrected CID checkpoint. Other learned-component removals that would require new full
training runs are outside the scope of the current evaluation.

\subsection{Final Training and Stage-B Reporting Rule}

The corrected run trains Stage A from the pinned base model under the contract summarized in
\cref{sec:evaluation-setup}. Stage A keeps
the frozen backbone in bfloat16 while retaining trainable CID modules in FP32 and using bfloat16
autocast for the forward pass; thresholded runtime probabilities are evaluated in FP32. Stage B then
unfreezes the backbone and uses FSDP~\citep{zhao2023fsdp}/AdamW~\citep{loshchilov2019adamw}. The selected Stage-B defaults are a CID peak learning rate of $2\times10^{-5}$, a backbone peak learning rate of $5\times10^{-6}$, 1\% linear warmup, a WSD stable phase, and a final 10\% linear decay to 10\% of peak.

\paragraph{Stage-B learning-rate calibration.}
We selected the CID-module peak rate with a short fixed-budget sweep from the same Stage-A checkpoint. All candidates used the corrected data-order sampler, WSD, the same seed and batch geometry, a fixed backbone peak rate of $5\times10^{-6}$, 2,400 rollout windows per rank, and the same 512-example fixed-noise validation set. The short probe is used only for configuration selection; formal model results use the subsequently trained final checkpoint.

\begin{table}[t]
  \centering
  \footnotesize
  \setlength{\tabcolsep}{3.0pt}
  \begin{tabular}{rrrrr}
    \toprule
    CID LR & Display exact & Need F1 & Conv. & Recovery fail. \\
    \midrule
    $\mathbf{2\times10^{-5}}$ & \textbf{41.40} & \textbf{15.70} & 90.79 & 4.17 \\
    $3\times10^{-5}$ & 37.02 & 13.47 & 90.63 & 4.22 \\
    $4\times10^{-5}$ & 39.11 & 12.75 & 88.89 & \textbf{3.95} \\
    $5\times10^{-5}$ & 39.67 & 14.96 & \textbf{91.96} & 4.03 \\
    \bottomrule
  \end{tabular}
  \caption{Short Stage-B learning-rate calibration. Metrics are fixed-seed free-rollout validation percentages; lower recovery failure is better.}
  \label{tab:stageb-lr-sweep}
\end{table}

The $2\times10^{-5}$ setting gives the strongest overall deployment-oriented profile, leading both materialized-display exact rate and Need F1 while retaining high convergence. We therefore use it as the Stage-B default and keep the backbone peak rate fixed at $5\times10^{-6}$.

To avoid reporting stale pilot settings as final hyperparameters, the final training table is populated only from frozen checkpoint metadata after the run completes. All formal performance claims therefore refer to one well-defined model and train--runtime contract.

\section{Open Technical Questions}
\label{app:questions}

Several design decisions remain open:
\begin{itemize}[leftmargin=*]
  \item which iterative process should update the thought tensor, how many logical cells different tasks need, and how aggressively allocation, retirement, reclamation, and compaction should be scheduled;
  \item whether display and thought diffusion should share a backbone and how source descriptors generalize to unseen tools;
  \item how best to calibrate direct local-noise deltas and \textsc{keep}/\textsc{reopen}/\textsc{stabilize} actions while preserving privacy during debugging;
  \item how to tune refinement-epoch and wall-clock budgets across deployment settings.
\end{itemize}
These questions remain open design variables within the CID framework.
