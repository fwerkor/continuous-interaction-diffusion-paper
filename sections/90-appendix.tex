...
The early diagnostic prototype uses \texttt{GSAI-ML/iLLaDA-8B-Base}~\citep{nie2026illada} at revision
...
The diagnostic prototype and corrected retraining draw from the released \texttt{fwerkor/CID-Dataset} semantic mixture~\cite{cao2026ciddataset}. The underlying tasks, prompts, evidence, and answers are kept fixed across the contract correction, while the corrected materialization may add contract-level supervision such as a stable information-need owner, additional affected TCT cells, affected display regions, and source-level protected-result policy. The pre-v3 release materialization is
...
TCT capacity is 128. Its recorded SHA-256 is
\texttt{bec927029445815f085f5597ac1e4519}\allowbreak\texttt{6e8c64995793e218bc7f73df52eb8492}; the corrected v3 materialization is frozen and recorded explicitly with the formal checkpoint, preventing accidental reuse of this pre-v3 hash.
...
The natural-public subset contributes 30,000 NQ-Open train tasks~\citep{kwiatkowski2019natural,lee2019latent} (CC-BY-SA-3.0), 4,500 OASST1 train
...
four-GPU phases used accumulation 8. The backbone parameters have \texttt{requires\_grad=False}. The forward and input-gradient path still traverses
the full decoder because CID projections precede it. Stage A is therefore materially cheaper in optimizer
state and parameter-gradient computation than Stage B while still exercising the full decoder path.
...
The corrected neural contract adds multi-label affected-cell and affected-display routing losses with
weights 0.2 and 0.2, respectively. Protected fact promotion remains outside the learned objective and
is controlled by the registered source/runtime policy.

\subsection{Early Diagnostic Protocol}

The development diagnostic uses 32 NQ-Open rows~\citep{kwiatkowski2019natural,lee2019latent} sampled with seed 20260824 from
...
\cref{sec:evaluation-setup}; it does not initialize from the early diagnostic prototype. Stage A keeps
